ST.cluster.leroux.Wchange.rho<-function(
  x,STy,offst,E,
  wpool,
  tripletf.pool.T,
  ...
  cur.Q.w<-list(t1=rho.initial*Wstar.pool.T[[1]][[w.initial[1]]] + (1 -rho.initial)*diag(1, n, n),
                t2=rho.initial*Wstar.pool.T[[2]][[w.initial[2]]] + (1 -rho.initial)*diag(1, n, n),
                t3=rho.initial*Wstar.pool.T[[3]][[w.initial[3]]] + (1 -rho.initial)*diag(1, n, n),
                t4=rho.initial*Wstar.pool.T[[4]][[w.initial[4]]] + (1 -rho.initial)*diag(1, n, n),
                t5=rho.initial*Wstar.pool.T[[5]][[w.initial[5]]] + (1 -rho.initial)*diag(1, n, n),
                t6=rho.initial*Wstar.pool.T[[6]][[w.initial[6]]] + (1 -rho.initial)*diag(1, n, n),
                t7=rho.initial*Wstar.pool.T[[7]][[w.initial[7]]] + (1 -rho.initial)*diag(1, n, n)
                )
  


  cur.det.Q.w<-list(t1=Qwdet(WstarValue=Wstar.value.pool.T[[1]][[w.initial[1]]],rho=rho.initial,tp=1),
                    t2=Qwdet(WstarValue=Wstar.value.pool.T[[2]][[w.initial[2]]],rho=rho.initial,tp=1),
                    t3=Qwdet(WstarValue=Wstar.value.pool.T[[3]][[w.initial[3]]],rho=rho.initial,tp=1),
                    t4=Qwdet(WstarValue=Wstar.value.pool.T[[4]][[w.initial[4]]],rho=rho.initial,tp=1),
                    t5=Qwdet(WstarValue=Wstar.value.pool.T[[5]][[w.initial[5]]],rho=rho.initial,tp=1),
                    t6=Qwdet(WstarValue=Wstar.value.pool.T[[6]][[w.initial[6]]],rho=rho.initial,tp=1),
                    t7=Qwdet(WstarValue=Wstar.value.pool.T[[7]][[w.initial[7]]],rho=rho.initial,tp=1)
                    )
  
  #### Start timer
    cat("Generating", "post burnin and thinned (if requested) samples.\n", sep = " ")
    progressBar <- txtProgressBar(style = 3)
    percentage.points<-round((1:100/100)*iterations)
  ...
  for (t in 2:iterations){
    ...
    for (timepoint in 1:tp){
      ...
      Wstar<- Wstar.pool.T[[timepoint]][[prop.w.index]]
      Wstar.value<-Wstar.value.pool.T[[timepoint]][[prop.w.index]]
      prop.Q.w <-cur.rho[timepoint]*Wstar + (1-cur.rho[timepoint])*diag(1, n, n)
      prop.det.Q.w <-Qwdet(WstarValue=Wstar.value,rho=cur.rho[timepoint],tp=1)
    ...
    ## update rho for each time period 
    for (timepoint in 1:tp){
      prop.rho<-rtruncnorm(1,a=0,b=0.9999999999,mean=cur.rho[timepoint],sd=sqrt(rho.proposal.v)) 
      prop.rho.Q.w<-prop.rho*Wstar.pool.T[[timepoint]][[cur.w.index[timepoint]]] + (1 -prop.rho)*diag(1, n, n)
      prop.rho.det.Q.w<-Qwdet(WstarValue=Wstar.value.pool.T[[timepoint]][[cur.w.index[timepoint]]],rho=prop.rho,tp=1)
      full.w<- cur.det.Q.w[[timepoint]]- 0.5*quadvalue[timepoint]/cur.tau2[timepoint]
      prop.quadvalue[timepoint]<-quadraticform(prop.rho.Q.w, ST.cur.phi[,timepoint],n)
      full.prop.w<-  prop.rho.det.Q.w- 0.5*prop.quadvalue[timepoint]/cur.tau2[timepoint]
      
      ratio.rho <- exp(full.prop.w - full.w)
      if(runif(1,0,1) < ratio.rho)
      {
        cur.rho[timepoint] <- prop.rho
        rho_mc[t,timepoint]<-cur.rho[timepoint]
        cur.Q.w[[timepoint]]<-prop.rho.Q.w
        cur.det.Q.w[[timepoint]]<-prop.rho.det.Q.w
        cntrho <- cntrho+1
        quadvalue[timepoint]<-prop.quadvalue[timepoint]
    ...
    if (floor(t/100)==t/100){   
      ...
      step.rho.acceptance.rate<-cntrho/(t*tp)
      if(step.rho.acceptance.rate<0.3){
        rho.proposal.v<-rho.proposal.v*(1/2)
      }else if
      (step.rho.acceptance.rate>0.6){
        rho.proposal.v<-rho.proposal.v*2
      } 
    ...
    ################################
    if(t %in% percentage.points)
    {
      setTxtProgressBar(progressBar, t/iterations)
    }
    
  } 
  
  ## store optimal proposal variance
  ...
  rho.acceptance.rate<-cntrho/(iterations*tp)
  ...
  if(burnin==0){
    ...
    rho_mc<-as.data.frame(rho_mc[-c(1:burnin),])
  }
  ...
  if (thinning==0){
    ...
    rho_mc<-rho_mc[seq(1,nrow(rho_mc),thinning),]
  }
  ...
  output<-list(beta.chain=as.data.frame(beta_mc),
               ...
               rho.chain=as.data.frame(rho_mc), 
               ...
               rho.acceptance.rate=rho.acceptance.rate,
               w.initial=w.initial
  )
  return(output)
}
