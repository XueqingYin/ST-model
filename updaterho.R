ST.cluster.leroux.rho<-function(
  x,STy,offst,E,
  wpool,#Q.w, detQ,
  tripletf.pool,
  neighbour_size.pool,
  ...
  cntw <- 0 
  cntrho<-0
  #phi_mc [1,]<-as.vector(STtruephi)  #as.vector(STtruephi)
  #tau2_mc[1,]<-truetau2
  
  
  tripletf<-tripletf.pool[[w.initial]]
  neighbour_size<-neighbour_size.pool[[w.initial]]
  #Wstar <- diag(neighbour_size) - wpool[,,w.initial]
  #Wstar.value <- eigen(Wstar)$values
  Wstar<- Wstar.pool[[w.initial]]
  Wstar.value<-Wstar.value.pool[[w.initial]]
  
  cur.Q.w <-rho.initial*Wstar + (1 -rho.initial)*diag(1, n, n)
  cur.det.Q.w <-as.numeric(0.5 * sum(log((rho.initial * Wstar.value + (1-rho.initial))))) #equal to 0.5*log(det(sim.Q.w[[j]]))
  cur.det.Q.w<-cur.det.Q.w*tp
  ...
  for (t in 2:iterations){
    ...
    updatephis<- updatephi2_initial(proposal_phi=ST.prop.phi, ###!!!! to avoid having the same name, this name is changed to updatephi2_initial
    ...
    Wstar<- Wstar.pool[[prop.w.index]]
    Wstar.value<-Wstar.value.pool[[prop.w.index]]
    prop.Q.w <-cur.rho*Wstar + (1-cur.rho)*diag(1, n, n)
    prop.det.Q.w <-Qwdet(WstarValue=Wstar.value,rho=cur.rho,tp=tp)
    ...
    ## update rho
    prop.rho<-rtruncnorm(1,a=0,b=0.9999999999,mean=cur.rho,sd=sqrt(rho.proposal.v))  
    


    Wstar<- Wstar.pool[[cur.w.index]]
    Wstar.value<-Wstar.value.pool[[cur.w.index]]
    prop.Q.w <-prop.rho*Wstar + (1-prop.rho)*diag(1, n, n)
    prop.det.Q.w <-Qwdet(WstarValue=Wstar.value,rho=prop.rho,tp=tp)
    ...
    if (floor(t/100)==t/100){   
      ...
      step.rho.acceptance.rate<-cntrho/t
  ...
  rho.acceptance.rate<-cntrho/iterations
